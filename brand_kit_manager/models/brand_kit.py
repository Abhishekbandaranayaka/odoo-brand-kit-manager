...
class BrandKit(models.Model):
    ...
    def _sync_company_brand_fields(self):
        self.ensure_one()
        company_fields = self.company_id._fields
        company_values = {}
        if "brand_kit_id" in company_fields:
            company_values["brand_kit_id"] = self.id
        if "brand_primary_color" in company_fields:
            company_values["brand_primary_color"] = self.primary_color or False
        if "brand_secondary_color" in company_fields:
            company_values["brand_secondary_color"] = self.secondary_color or False
        if "brand_footer_text" in company_fields:
            company_values["brand_footer_text"] = self.footer_text or False
        if "brand_legal_text" in company_fields:
            company_values["brand_legal_text"] = self.legal_text or False
        if "brand_email_header_text" in company_fields:
            company_values["brand_email_header_text"] = self.email_header_text or False
